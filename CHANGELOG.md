...
- **输出改为纯计算结果，不再给估值定性**。此前工具会输出「极度低估 / 低估 / 合理 / 偏贵 / 高估」
  这类带倾向的估值判断——那已经越过「数据计算」进入「对具体证券的投资分析意见」。现在：
  - **AI 提示词**：`### 7. 综合结论`（PEG 评级 + 估值判断 + 一句话核心观点）改为
    `### 7. 计算结果汇总`（PEG 数值 / 前瞻 PE / CAGR / PE 消化年数 / 同行数值区间）；
    区间表由「PEG < 0.5 → 极度低估」改为「PEG < 1.0 → 前瞻 PE 低于盈利增速」这类纯数值关系描述；
    并显式要求不使用定性或倾向性表述、不预测走势、不给买卖时机或目标价。
  - **看板标签**（`WatchlistTable`）：由程序按 PEG 打的「低估 / 高估」标签，改为
    `PE < 增速` / `PE ≈ 增速` / `PE > 增速` 的数值关系描述。
  - **报告徽标**与索引回填：只显示计算出的 PEG 数值（如 `PEG 0.86`），不再抽取定性词。
- README 的 PEG 区间表同步改为数值关系表述，并说明本工具只输出计算结果、
  估值是否合适由使用者自行判断。
- `web/package-lock.json` 版本号与 `package.json` 对齐（此前不同步）。
